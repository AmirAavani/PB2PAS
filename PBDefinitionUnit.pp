...
type
  ...
  public
    ...
    function ToString(Indent: AnsiString): AnsiString; override;
  end;
  ...
  TImports = TStringList;
  TPackages = TStringList;
  ...
  public
    constructor Create(_Syntax: AnsiString; _Imports: TImports; _Packages: TPackages;
...
function TOneOf.ToString(Indent: AnsiString): AnsiString;
var
  OOField: TOneOfField;

begin
  Result:= Indent;
  Result += Format('OneOf %s Options: %s (',
    [GetName, GetOptions.ToString('')]);

  for OOField in Self.OneOfFields do
    Result += #10 + Indent + OOField.ToString(Indent + '.');
  Result += #10 + Indent + ')';
...
begin
  Result:= Format('%s Message'#10'%s Name = %s'#10'%s Options: ('#10'%s'#10')'#10'%s Fields: ('#10+ '%s'#10') '#10+
    '%s Messages: ('#10'%s'#10')'#10'%s Enums: ('#10'%s'#10')',
    [Indent, Indent, Name, Indent, Options.ToString(''),
...
begin
  ...
  if GetOptions <> nil then
    OptionsString := GetOptions.ToString('');
  Result += Format('%s %s = %d (%s)',
    [GetFieldType, GetName, GetFieldNumber,
      IfThen(GetOptions <> nil, OptionsString, '')]);
...
constructor TProto.Create(_Syntax: AnsiString; _Imports: TImports;
